...
| --- | --- | --- | --- |
| GA4 | Production ID `G-EPSJV6EMLC` is configured; a prior browser run observed a consent-gated `page_view` request to Google Analytics. | Two consented production contact submissions completed on 2026-09-02 and the GA loader was present, but the realtime UI did not yet expose named event evidence during the check. | `configured`, event evidence pending |
