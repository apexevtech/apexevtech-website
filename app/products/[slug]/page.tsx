    ...
    <>
      ...
      <section className="px-5 py-16 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="flex min-h-[460px] items-center justify-center rounded-lg border border-slate-200 bg-gradient-to-br from-blue-50 to-cyan-50 p-8">
          ...
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {product.highlights.map((highlight) => (
                <div key={highlight} className="rounded-lg border border-slate-200 bg-white p-4 font-extrabold text-slate-900">
                  {highlight}
                </div>
              ))}
            </div>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href="/contact" className="rounded-md border border-blue-200 px-5 py-3 font-extrabold text-apex-blue hover:bg-blue-50">
                Send Inquiry
              </Link>
            </div>
          </div>
              ...
              <ul className="divide-y divide-slate-100">
                {product.features.map((feature, index) => (
                  <li key={feature} className="flex gap-3 py-3 first:pt-0 last:pb-0">
                    <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-blue-50 text-[10px] font-black text-apex-blue">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-5 md:grid-cols-4">
            {product.applications.map((application) => (
              <div key={application} className="rounded-lg bg-slate-950 p-6 font-black text-white">
                {application}
              </div>
            ))}
          </div>
