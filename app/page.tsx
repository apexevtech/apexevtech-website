    ...
    <>
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto grid max-w-7xl items-center gap-6 px-5 py-6 sm:gap-8 sm:py-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-10 lg:px-8 lg:py-12">
          <div className="max-w-2xl">
            <p className="text-sm font-black uppercase text-[#1268a8]">APEX Power Systems · EVSE testing</p>
            <h1 className="mt-4 text-4xl font-black leading-[1.08] text-[#12263a] md:text-5xl lg:text-[60px]">
              EV charger test equipment for laboratory and field validation.
            </h1>
            <p className="mt-5 max-w-xl text-base leading-7 text-[#526b7d] sm:mt-6 sm:text-lg sm:leading-8">
              Apex Power Systems (Nanjing) Co., Ltd. supplies AC and DC EVSE test systems for charger development, production verification and field commissioning. Match the equipment to your connector, electrical range and test workflow.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/contact#inquiry-form" className="inline-flex items-center rounded-md bg-[#1479c9] px-6 py-3.5 text-sm font-extrabold text-white transition-colors hover:bg-[#0f5f9f]">
                Discuss your requirements <span className="ml-2" aria-hidden="true">→</span>
              </Link>
              <Link href="/products" className="inline-flex items-center rounded-md border border-slate-300 bg-white px-6 py-3.5 text-sm font-extrabold text-[#12263a] transition-colors hover:border-[#1479c9] hover:text-[#1268a8]">
                Browse test systems
              </Link>
            </div>
      ...
      <section className="border-b border-slate-200 bg-[#f3f6f8]">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-x-8 gap-y-3 px-5 py-5 text-sm font-bold text-[#385064] lg:px-8">
          <span className="text-[#1268a8]">Built for</span>
          ...
          <div className="grid border-l border-t border-slate-200 md:grid-cols-2 lg:grid-cols-4">
            {capabilities.map((capability) => (
              <article key={capability.index} className="min-h-64 border-b border-r border-slate-200 bg-white p-6">
                <span className="text-sm font-black text-[#067187]">{capability.index}</span>
          ...
          <ol className="border-t border-slate-200">
            {process.map(([number, title, description]) => (
              <li key={number} className="grid gap-3 border-b border-slate-200 py-6 sm:grid-cols-[52px_180px_1fr] sm:items-start">
                <span className="font-black text-[#067187]">{number}</span>
      ...
      <section className="bg-[#102a43] px-5 py-12 text-white lg:px-8">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-6 md:flex-row md:items-center">
          ...
          <Link href="/contact#inquiry-form" className="inline-flex shrink-0 self-start rounded-md bg-white px-6 py-3.5 text-sm font-extrabold text-[#102a43] hover:bg-[#e8f7fa]">
